...
| v0.6.0-alpha.1 legacy baseline action apply/verify/rollback and crash recovery | passed | passed | high-risk confirmation supplied explicitly by the VM harness |
...
| Baseline catalog actions: Inspect, Capture, Apply, Verify, exact Restore | historical pass on build 26200/19045 | historical pass | repeat before stable release; current page-file/core-parking/per-app GPU writers remain pending |
...
The detailed Windows 11 integrity harness then exercised the legacy baseline actions separately with deliberately mixed original Registry states (DWORD, QWORD, string, and absent values). Those cases passed Inspect, Apply, Verify, raw read-back, Registry export, apply/rollback restore points, exact value-and-kind rollback, and final manifest validation. The newly added page-file, core-parking, and dynamic per-app GPU writers are not claimed as passed until the disposable Windows 11 matrix is rerun.
...
not satisfy their current validation gate. The Windows 11 VM was temporarily started with 4 GB because the
