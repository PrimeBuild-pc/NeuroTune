...
function PlanItemReview({ recommendations }: { recommendations: Recommendation[] }) {
  ...
  return <section className="section-card plan-item-section"><div className="section-heading"><div><span className="eyebrow">Review-only plan items</span><h3>Guidance, scripts, resources, and notices</h3></div><span className="status-pill">Never auto-run</span></div><div className="plan-item-list">{reviewOnly.map(item => <article key={item.id} className={`plan-item ${item.kind}`}><div className="plan-item-heading"><FileText size={18}/><div><span>{planKindLabel(item.kind)}</span><strong>{item.title}</strong></div><span className={`risk ${item.risk}`}>{item.risk} risk</span></div><p>{item.reason}</p>{item.expectedImpact && <small><strong>Expected impact:</strong> {item.expectedImpact}</small>}{item.tradeoffs.length > 0 && <small><strong>Trade-offs:</strong> {item.tradeoffs.join(' · ')}</small>}{item.reviewWarnings.length > 0 && <div className="script-warnings" role="alert">{item.reviewWarnings.map(warning => <span key={warning}>{warning}</span>)}</div>}{item.kind === 'scriptArtifact' && <><pre className="script-preview">{item.script}</pre><div className="button-row"><button className="secondary" onClick={() => void navigator.clipboard.writeText(item.script)}><Copy size={15}/>Copy</button><button className="secondary" onClick={() => saveScriptArtifact(item)}><Download size={15}/>Save as text</button></div></>}{item.sourceReferences.length > 0 && <div className="source-list">{item.sourceReferences.map(source => <a key={`${item.id}-${source.url}-${source.title}`} href={source.url || undefined} target="_blank" rel="noreferrer">{source.title} · {source.grade}</a>)}</div>}<div className="conflict-evidence">{item.evidenceIds.map(id => <code key={id}>{id}</code>)}</div></article>)}</div></section>;
}
